...
LARGE_DEVIATION_THRESHOLD_FOR_PIXEL = 0.25
...
def sum_phase_values_for_each_loop(ifg_files: List[str], loops: List[List[SignedEdge]],
    ...
    for k, loop in enumerate(loops):
        for signed_edge in loop:
            ...
            index = edge_to_indexed_ifgs[signed_edge.edge].index
...
closure = sum_phase_values_for_each_loop(ifg_files, retained_loops)
