...
def sum_phase_closures(
    ...
    if params[C.PARALLEL]:
        ...
        closure = np.zeros(shape=(*ifg_shape, len(loops)), dtype=np.float32)
        ...
        closure_process = np.zeros(shape=(* ifg_shape, len(process_loops)), dtype=np.float32)
        ...
        if mpiops.rank == 0:
            ...
            for rank in range(1, mpiops.size):
                ...
                rank_closure = np.zeros(shape=(*ifg_shape, len(rank_indices)), dtype=np.float32)
                mpiops.comm.Recv(rank_closure, source=rank, tag=rank)
                closure[:, :, rank_indices] = rank_closure
        else:
            closure = None
        ...
        if mpiops.MPI_INSTALLED:
            mpiops.comm.Reduce(
                [ifgs_breach_count_process, mpiops.MPI.UINT16_T],
                [ifgs_breach_count, mpiops.MPI.UINT16_T],
                op=mpiops.MPI.SUM,
                root=0
            )  # global
        else:
            ifgs_breach_count = mpiops.comm.reduce(
                ifgs_breach_count_process,
                op=mpiops.sum0_op,
                root=0
            )

        log.debug("successfully summed phase closure breach array")

    num_occurrences = None
    if mpiops.rank == 0:
        num_occurrences = _find_num_occurrences_each_ifg(loops, edge_to_indexed_ifgs, n_ifgs)

    return closure, ifgs_breach_count, num_occurrences


def _find_num_occurrences_each_ifg(
...
def __compute_ifgs_breach_count(
    ...
    # i.e., nans are not selected in indices_breaching_threshold
    ...
    for signed_edge in weighted_loop.loop:
        ...
        # as we don't know the phase of these pixels and also they were converted to
        # zero before closure check.   Therefore, we leave them out of ifgs_breach_count,
        # i.e., we don't increment their ifgs_breach_count values
