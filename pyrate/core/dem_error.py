...
"""
This Python module implements the calculation of correction for residual
topographic effects (a.k.a. as DEM errors).
"""
...
def dem_error_calc_wrapper(params: dict) -> None:
    ...
    if mpiops.run_once(__check_and_apply_demerrors_found_on_disc, ifg_paths, params):
        ...
        # TODO: subtract other corrections (e.g. orbital) from displacement
        # phase before estimating the DEM error

        # the following code is a quick way to do the bperp calculation, but is not identical to
        # the GAMMA output where the near range of the first SLC is used for each pair.
...
def _process_dem_error_per_tile(tile: Tile, params: dict) -> None:
    ...
    # current implementation uses the look angle and range distance matrix of
    # the primary SLC in the last IFG
    ...
    # dem_error contains the estimated DEM error for each pixel
    # (i.e. the topographic change relative to the DEM)
    # size [row, col]

    # dem_error_correction contains the correction value for each interferogram
    # size [num_ifg, row, col]
    # save tiled data in tmpdir
    np.save(file=tmp_dir / f'dem_error_{tile.index}.npy', arr=dem_error)
    ...
    np.save(file=tmp_dir / f'dem_error_correction_{tile.index}.npy', arr=tmp_array)

    # Calculate and save the average perpendicular baseline for the tile
    bperp_avg =  np.nanmean(bperp, axis=(1, 2), dtype=np.float64)
    np.save(file=tmp_dir / f'bperp_avg_{tile.index}.npy', arr=bperp_avg)
...
def _calculate_bperp_wrapper(ifg_paths: list, az_parts: np.ndarray, rg_parts: np.ndarray,
    ...
    for ifg_num, ifg_path in enumerate(ifg_paths):
        ...
        look_angle, _, _, range_dist = geometry.calc_pixel_geometry(
            ifg, rg_parts, lon_parts, lat_parts, dem_parts
        )
...
def calc_dem_errors(ifgs: list, bperp: np.ndarray, look_angle: np.ndarray, range_dist: np.ndarray,
    ...
    """
    Function to calculate the DEM error for each pixel using least-squares adjustment of phase
    data and perpendicular baseline. The least-squares adjustment co-estimates the velocities.
    ...
    :return: dem_error_correction: DEM error correction for each pixel and interferogram
        (nifgs x nrows x ncols)
    :return: vel: velocity estimate for each pixel (nrows x ncols)
    """
    ifg_data, mst, ncols, nrows, ifg_time_span = _per_tile_setup(ifgs)
    if threshold < 4:
        msg = "pixel threshold too low (i.e. <4) resulting in non-redundant DEM error estimation"
        raise DEMError(msg)
        ...
        for col in range(ncols):
            ...
            if len(sel) >= threshold:  # given threshold for number of valid pixels in time series
                ...
                if np.isnan(y).any() or np.isnan(bperp_pix).any():
                    continue
                # using the actual geometry of a particular IFG would be possible but is likely not
                ...
                m = len(sel)
                # Design matrix of least-squares system, velocities are co-estimated as
                # done in StaMPS or MintPy
                A = np.column_stack((np.ones(m), bperp_pix, time_span))
                # solve ordinary least-squares system using numpy.linalg.lstsq function
                xhat, _, _, _ = np.linalg.lstsq(A, y, rcond=None)
    ...
    # calculate correction value for each IFG by multiplying the
    # least-squares estimate with the Bperp value
    ...
    #
    # also scale by -0.001 since the phase observations are in mm with
    # positive values away from the sensor
...
def _per_tile_setup(ifgs: list) -> Tuple[np.ndarray, np.ndarray, int, int, np.ndarray]:
    ...
    :return: mst: an array of booleans representing valid ifg connections
        (i.e. the minimum spanning tree)
...
def _write_dem_errors(ifg_paths: list, params: dict, preread_ifgs: dict) -> None:
    """
    Convenience function for writing DEM error (one file) and DEM error correction
    for each IFG to disc.
    ...
    for ifg_path in ifg_paths:
        ...
        dem_error_correction_ifg = assemble_tiles(
            shape, params[C.TMPDIR], tiles,
            out_type='dem_error_correction', index=idx
        )
...
def __check_and_apply_demerrors_found_on_disc(ifg_paths: list, params: dict) -> bool:
    """
    Convenience function to check if DEM error correction files have already been
