...
def dem_error_calc_wrapper(params: dict) -> None:
    ...
    #print(bperp.shape)
      ...
      for t in process_tiles:
        ...
        for ifg_path in ifg_paths: # loop can be avoided by approximating the look angle for the first Ifg
            ifg = Ifg(ifg_path)
...
#        ifg_parts = [shared.IfgPart(p, t, preread_ifgs, params) for p in ifg_paths]
#        bperp_parts = bperp[:, t.top_left_y:t.bottom_right_y, t.top_left_x:t.bottom_right_x]
        ...
        dem_error, dem_error_correction = calc_dem_errors(ifg_parts, bperp, vcmt)
...
def calc_dem_errors(ifgs, bperp, vcmt=None):
    ...
    p_thresh = 3 # -> to avoid singularities
...
def __calc_dem_error_for_tile(tile, params, bperp):
    """
    Helper function to calculate  setting up time series computation parameters

    :param object tile: tile to be processed
    :param dict params: Dictionary of configuration parameters
    :param ndarray bperp: Per-pixel perpendicular baseline for each interferogram
    """
    ...
    vcmt = params[cf.VCMT]
    ifg_paths = [ifg_path.tmp_sampled_path for ifg_path in params[cf.INTERFEROGRAM_FILES]]
    log.debug(f"Calculating DEM error for tile {tile.index}")
    ...
    # save tiled data in tmpdir
    np.save(file=os.path.join(params[cf.TMPDIR], 'dem_error_{}.npy'.format(tile.index)), arr=dem_error)
    # swap the axes of 3D array to fit the style used in function assemble_tiles
    tmp_array = np.moveaxis(dem_error_correction, 0, -1)
    # new dimension is [row, col, num_ifg]
    # save tiled data into tmpdir
    np.save(file=os.path.join(params[cf.TMPDIR], 'dem_error_correction_{}.npy'.format(tile.index)), \
            arr=tmp_array)
