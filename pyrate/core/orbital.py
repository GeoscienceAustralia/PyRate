...
from pyrate.core.algorithm import main_subordinate_ids, get_all_epochs
...
def network_orbital_correction(ifgs, degree, offset, params, m_ifgs: Optional[List] = None,
    ...
    if preread_ifgs:
        ...
        ids = main_subordinate_ids(get_all_epochs(temp_ifgs))
    else:
        ids = main_subordinate_ids(get_all_epochs(ifgs))
...
def _remove_network_orb_error(coefs, dm, ifg, ids, offset):
    ...
    """
    orb = dm.dot(coefs[ids[ifg.subordinate]] - coefs[ids[ifg.main]])
...
def get_network_design_matrix(ifgs, degree, offset):
    ...
    for i, ifg in enumerate(ifgs):
        ...
        m = ids[ifg.main] * ncoef  # start col for main
        s = ids[ifg.subordinate] * ncoef  # start col for subordinate
