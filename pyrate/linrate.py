...
Based on original Matlab code by Hua Wang and Juliet Biggs, and Matlab 'lscov' function.

Author: Matt Garthwaite, GA
'''
from scipy.linalg import solve, cholesky, qr, inv
from numpy import empty, nan, isnan, shape, mod, sqrt, diag, delete, ones, array, nonzero, float32

def linear_rate(ifgs, vcm, pthr, nsig, maxsig, mst=None):
	'''
	Pixel-by-pixel linear rate (velocity) estimation using iterative weighted least-squares method.
	
	ifgs: Sequence of ifg objs from which to extract observations
	vcm: Derived positive definite temporal variance covariance matrix
	pthr: Pixel threshold; minimum number of coherent observations for a pixel
	nsig: n-sigma ratio used to threshold 'model minus observation' residuals
	maxsig: Threshold for maximum allowable standard error
	mst: Pixel-wise matrix describing the minimum spanning tree network
	'''
