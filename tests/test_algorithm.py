...
class DateLookupTests(TestCase):
    ...
    def test_ifg_date_lookup(self):
        # check reverse lookup of ifg given a main and subordinate date tuple
...
class EpochsTests(TestCase):
    ...
    def test_main_subordinate_ids(self):
        d0 = date(2006, 6, 19)
        d1 = date(2006, 8, 28)
        d2 = date(2006, 10, 2)
        d3 = date(2006, 11, 6)
        ...
        self.assertEqual(exp, main_subordinate_ids([d3, d0, d2, d1]))
        self.assertEqual(exp,
                         main_subordinate_ids([d3, d0, d2, d1, d3, d0]))
