...
class TestGammaVsRoipacEquality(unittest.TestCase):
    ...
    def check_gamma(self, conf_file):
        ...
        self.make_gamma_input_files(data_paths, conf_file)
    ...
    def make_roipac_input_files(self, data, conf_file, projection):
        """ """
        with open(conf_file, "w") as conf:
            conf.write("[{}]\n".format(DUMMY_SECTION_NAME))
    ...
    def test_cmd_ifg_no_roipac_files_created_roipac(self):
        data_paths = common.small_data_roipac_unws()
        ...
        self.check_roipac(data_paths)

    def check_roipac(self, data_paths):
        conf_file = self.roipac_conffile
