...
class SinglePixelIfg(object):
    ...
    def __init__(self, main, subordinate, phase, nan_fraction):
...
class TimeSeriesTests(unittest.TestCase):
    ...
    def test_time_series_unit(self):
        ...
        """
        imain = asarray([1, 1, 2, 2, 3, 3, 4, 5])
        isubordinate = asarray([2, 4, 3, 4, 5, 6, 6, 6])
        ...
        dates.sort()
        main = [dates[m_num - 1] for m_num in imain]
        subordinate = [dates[s_num - 1] for s_num in isubordinate]

        self.ifgs = [SinglePixelIfg(m, s, p, n) for m, s, p, n in
                     zip(main, subordinate, phase, nan_fraction)]
