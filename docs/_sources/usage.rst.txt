...
Example configuration files for running PyRate with GAMMA or ROI\_PAC
format interferograms are contained in the *configs/* directory.
...
instructions to use a HPC cluster can be found in the `hpc
guide <hpc>`__.
...
multi-threading simply by turning ``parallel:  1`` in the configuration
