...
class AlgorithmTests(TestCase):
	...
	def test_utm_zone(self):
		...
		for lon in [174.0, 176.5, 179.999, 180.0]:
			self.assertEqual(60, algorithm.utm_zone(lon))

		for lon in [144.0, 144.1, 146.3456, 149.9999]:
			self.assertEqual(55, algorithm.utm_zone(lon))

		for lon in [-180.0, -179.275, -176.925]:
			self.assertEqual(1, algorithm.utm_zone(lon))

		for lon in [-72.0, -66.1]:
			self.assertEqual(19, algorithm.utm_zone(lon))

		for lon in [0.0, 0.275, 3.925, 5.999]:
			self.assertEqual(31, algorithm.utm_zone(lon))
	...
	def test_cell_size_polar_region(self):
		...
		for lat in [-80.1, -85.0, -90.0, 84.1, 85.0, 89.9999, 90.0]:
			self.assertRaises(ValueError, algorithm.cell_size, lat, 0, 0.1, 0.1)
	...
	def test_cell_size_calc(self):
		...
		for lon, lat in latlons:
			xs, ys = algorithm.cell_size(lat, lon, x_deg, y_deg)
			for s in (xs, ys):
